...
function validateApiKey() {
    ...
    // Check gemini-proxy.js
    total++;
    if (checkFile('gemini-proxy.js', 'Backend server (gemini-proxy.js)')) {
        score++;
        try {
            const proxyContent = fs.readFileSync('gemini-proxy.js', 'utf8');
            if (proxyContent.includes('GEMINI_API_KEY')) {
                log('  ✓ Backend configured to use API Key', 'green');
            }
            if (proxyContent.includes('generativelanguage.googleapis.com')) {
                log('  ✓ Backend configured to call Gemini API', 'green');
            }
        } catch (e) {
            log(`  ⚠️  Could not read gemini-proxy.js: ${e.message}`, 'yellow');
        }
    }
...
function validateMcpServer() {
    ...
    if (checkFile('kali-mcp-server.js', 'Kali MCP Server')) {
        ...
        try {
            ...
            const toolEndpoints = (mcpContent.match(/app\.post\('\/api\/tools\//g) || []).length;
    ...
    // Check MCP proxy configuration in gemini-proxy.js
    total++;
    if (fs.existsSync('gemini-proxy.js')) {
        score++;
        try {
            const proxyContent = fs.readFileSync('gemini-proxy.js', 'utf8');
            
            if (proxyContent.includes('/api/kali')) {
                log('✓ Backend configured to proxy MCP requests', 'green');
            }
            
            const kaliEndpoint = proxyContent.match(/KALI_MCP_ENDPOINT[^\n]+/);
            if (kaliEndpoint) {
                log(`  ✓ Kali endpoint configured: ${kaliEndpoint[0]}`, 'green');
            }
...
function generateReport(apiResult, mcpResult, taskResult) {
    ...
    log(`\n🔑 API Key Configuration:      ${apiResult.score}/${apiResult.total}`, apiResult.score === apiResult.total ? 'green' : 'yellow');
    log(`🛡️  MCP Server Configuration:   ${mcpResult.score}/${mcpResult.total}`, mcpResult.score === mcpResult.total ? 'green' : 'yellow');
    log(`👤 User Task Handling:         ${taskResult.score}/${taskResult.total}`, taskResult.score === taskResult.total ? 'green' : 'yellow');
    
    log(`\n📈 Overall Score: ${totalScore}/${totalPossible} (${percentage}%)`, percentage >= 80 ? 'green' : percentage >= 60 ? 'yellow' : 'red');
...
function main() {
    ...
    log('  • Run "npm test:api" to test the API (requires servers running)', 'blue');
    log('');
