...
## Technical Standards
- **Neumorphism**: All UI changes must follow the `.arsenal-*` class naming convention defined in `styles.css`.
- **Infrastructure**: Primary deployment target is AWS EC2 (Amazon Linux 2023). Avoid serverless for core tools.
- **Security**: Strict adherence to the Tamil Nadu Police Cybersecurity Department's authorized protocols. No "demo" or "mock" implementations.
...
- **Authorized Servers**:
  - `filesystem`: For deep codebase analysis and file manipulation.
  - `firebase`: For managing project data and authentication.
  - `genkit`: For AI logic development and flow management.
  - `memory`: For persistent project-specific knowledge recall.
  - `netlify`: For deployment and site management.
  - `notion`: For project documentation and task tracking.
  - `perplexity`: For real-time technical research and security verification.
- **Workflow**: If a task requires external data or system interaction, prioritize the use of these MCP servers.
