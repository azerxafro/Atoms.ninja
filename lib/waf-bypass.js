...
// ═══════════════════════════════════════════════
//  Technique 1: WAF Detection (wafw00f + fallbacks)
// ═══════════════════════════════════════════════

// WAF header/cookie/body signatures for fallback detection
const WAF_SIGNATURES = [
  { name: "Cloudflare", headers: ["cf-ray", "cf-cache-status", "cf-request-id"], server: "cloudflare" },
  { name: "AWS CloudFront", headers: ["x-amz-cf-id", "x-amz-cf-pop"], server: "cloudfront" },
  { name: "AWS WAF", headers: ["x-amzn-waf-"], server: "awselb" },
  { name: "Akamai", headers: ["x-akamai-transformed", "akamai-origin-hop"], server: "akamaighost" },
  { name: "Sucuri", headers: ["x-sucuri-id", "x-sucuri-cache"], server: "sucuri" },
  { name: "Imperva/Incapsula", headers: ["x-iinfo", "x-cdn"], cookies: ["incap_ses_", "visid_incap_"] },
  { name: "F5 BIG-IP ASM", headers: ["x-wa-info"], cookies: ["TS0", "BIGipServer"] },
  { name: "Barracuda", headers: ["barra_counter_session"] },
  { name: "Citrix NetScaler", headers: ["cneonction", "nncoection"], cookies: ["ns_af", "citrix_ns_id"] },
  { name: "Fortinet FortiWeb", headers: ["fortiwafsid"], cookies: ["cookiesession1"] },
  { name: "DDoS-Guard", headers: ["ddos-guard"], server: "ddos-guard" },
  { name: "StackPath", headers: ["x-sp-"], server: "stackpath" },
  { name: "Fastly", headers: ["x-fastly-request-id", "fastly-io-info"], server: "fastly" },
  { name: "Varnish", headers: ["x-varnish"], server: "varnish" },
  { name: "KeyCDN", headers: ["x-pull", "x-edge-location"], server: "keycdn" },
  { name: "Vercel", headers: ["x-vercel-id", "x-vercel-cache"], server: "vercel" },
];

async function detectWAF(domain) {
  const result = { technique: "waf_detection", status: "running", data: { method: [] } };
  try {
    // ── Layer 1: Try wafw00f if installed ──
    const waf = await runCommand("wafw00f", [domain, "-o", "-"], 20000);
    if (waf.code === 0 && waf.stdout && !waf.stderr.includes("not found")) {
      result.data.method.push("wafw00f");
      result.data.wafw00fRaw = waf.stdout.substring(0, 1500);
      ...
      result.data.wafw00fAvailable = false;
    }

    // ── Layer 2: HTTP header fingerprinting (always runs as enrichment) ──
    result.data.method.push("header_fingerprint");
    ...
    if (curl.stdout) {
      result.data.responseHeaders = curl.stdout;
      const hdrs = curl.stdout.toLowerCase();

      for (const sig of WAF_SIGNATURES) {
        // Check headers
        const headerMatch = sig.headers?.some((h) => hdrs.includes(h.toLowerCase()));
        // Check server header
        const serverMatch = sig.server && hdrs.includes(`server: ${sig.server}`) || hdrs.includes(sig.server || "");
        // Check Set-Cookie for WAF cookies
        const cookieMatch = sig.cookies?.some((c) => hdrs.includes(c.toLowerCase()));

        if (headerMatch || serverMatch || cookieMatch) {
          result.data.detected = true;
          if (!result.data.vendor) result.data.vendor = sig.name;
          if (!result.data.allDetected) result.data.allDetected = [];
          result.data.allDetected.push(sig.name);
        }
      }

      // Extract specific identifiers
      const cfRay = curl.stdout.match(/cf-ray:\s*(.+)/i)?.[1]?.trim();
      if (cfRay) result.data.cfRay = cfRay;
      const serverHdr = curl.stdout.match(/server:\s*(.+)/i)?.[1]?.trim();
      if (serverHdr) result.data.server = serverHdr;
    }

    // ── Layer 3: Trigger-based detection (send malicious request to provoke WAF block page) ──
    result.data.method.push("trigger_probe");
    const triggerCurl = await runCommand("curl", [
      "-s", "-m", "10", "--connect-timeout", "5", "-o", "/dev/null", "-w", "%{http_code}",
      "-A", "Mozilla/5.0",
      `https://${domain}/?id=1%27%20OR%201=1--&<script>alert(1)</script>`,
    ], 15000);

    if (triggerCurl.stdout) {
      const triggerStatus = triggerCurl.stdout.trim();
      result.data.triggerStatus = triggerStatus;
      // 403/406/429/503 on a malicious request strongly suggests WAF
      if (["403", "406", "429", "503"].includes(triggerStatus)) {
        result.data.detected = true;
        result.data.blocksBadRequests = true;
      }
    }

    // ── Layer 4: IP-range-based CDN detection ──
    result.data.method.push("ip_range_check");
    try {
      const ips = await dns.resolve4(domain);
      if (ips.length > 0) {
        result.data.resolvedIPs = ips;
        for (const ip of ips) {
          const cdn = detectCDN(ip);
          if (cdn) {
            result.data.detected = true;
            if (!result.data.vendor) result.data.vendor = cdn;
            result.data.cdnByIP = cdn;
            break;
          }
        }
      }
    } catch (e) { /* DNS resolution failed */ }

    // Default if nothing detected
    if (result.data.detected === undefined) {
      result.data.detected = false;
      result.data.vendor = null;
    }
