...
const app = express();
const KALI_BACKEND =
  process.env.ATOMS_EC2_ENDPOINT || "http://136.113.58.241:3001";
const PORT = process.env.PORT || 3001;

// Your Gemini API Key - Default locked key
const GEMINI_API_KEY =
  process.env.GEMINI_API_KEY || "AIzaSyAfftnDlIXG9eVfWVNfXDyhvrASjSloBIE";
...
app.post("/api/kali/tools/:tool", async (req, res) => {
  try {
    ...
    const kaliResponse = await fetch(`${KALI_BACKEND}/api/tools/${tool}`, {
      method: "POST",
    ...
    if (!kaliResponse.ok) {
      const error = await kaliResponse
        .json()
...
app.post("/api/kali", async (req, res) => {
  try {
    ...
    const kaliResponse = await fetch(`${KALI_BACKEND}/api/execute`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (error.name === "AbortError") {
      res
        .status(504)
        .json({ error: "Request timeout - tool took too long to execute" });
    } else {
      res.status(500).json({ error: error.message });
    }
  }
...
app.get("/api/kali/tools", async (req, res) => {
  try {
    const kaliResponse = await fetch(`${KALI_BACKEND}/api/tools`);
    const data = await kaliResponse.json();
    res.json(data);
  } catch (error) {
...
app.listen(PORT, () => {
  ...
  console.log(`🔧 Kali Backend: ${KALI_BACKEND}`);
